...
struct p2PQueue_item
{
	...
	p2PQueue_item(const tdata& _data, int order) : data(_data), priority(priority)
	{}
};
...
public:
	...
	* push new item
	*/
	void Push(const tdata& item, int order)
	{
		p2PQueue_item<tdata>*   p_data_item;
		p_data_item = new p2PQueue_item < tdata >(item, order);
